...
theorem planar_model_lsv_of_numerics
    ...
    (m : PlanarBandModel (n + 1) W c C L)
    (hseed : 8 ≤ seedSize (n + 1) W)
    (num : NumericalCertificate (n + 1) (blockCount (n + 1) W) (retainedRows (n + 1) W)
      W kappa R Kz A C1 (1 / (64 * Cw)) Cw)
    (entropy : CorrectedSection5NumericalConditions (n + 1) W kappa
      (blockCount (n + 1) W) 28 (1 / (64 * Cw)))
    (gap : Real.log (dimensionLossColumnPrefactor (n + 1) W) ≤
      Section5Formalization.finalExponentGap (n + 1) W kappa)
  ...
  have hcol : ∀ j, m.law (closedColumnEvent (fun omega => shifted (m.matrix omega) z) s j ∩ good) ≤
    ...
    have hlin : min 1 (Real.pi * (((n + 1 : Nat) : Real) * L / (c / W * d ^ 2)) * s ^ 2) ≤
      ...
      dsimp [D]
      apply planar_column_linearization
      all_goals first | assumption | positivity
    calc
  ...
  calc
    m.law (leastSingularBadEvent (fun omega => shifted (m.matrix omega) z)
      ...
      lsv_probability_from_cover (by omega) m.law m.matrix z R (tau (n + 1) W kappa t)
...
theorem eventually_planar_band_lsv
  ...
  | succ n =>
      exact planar_model_lsv_of_numerics (m (n + 1)) hN.1 hN.2.1 hN.2.2.1 hN.2.2.2
